...
import {
  ...
  getOnboardingCompleteCandidates,
  getCoachingTipCandidates,
...
async function collectCandidatesForUser(userId: string, enabled: EnabledCategories): Promise<NotificationCandidate[]> {
  const groups = await Promise.all([
    ...
    enabled.businessNotifications ? getOnboardingCompleteCandidates(userId) : Promise.resolve([]),
    enabled.businessNotifications ? getCoachingTipCandidates(userId) : Promise.resolve([]),
